// =====================================================================
//  r05_cosinus.js — 4ᵉ : cosinus dans le triangle rectangle.
...
export default {
  ...
  intro:
    ...
    "de départ de toute la trigonométrie de 3ᵉ (où s'ajoutent le sinus et la tangente). On travaille ici la " +
  ...
  exercices: [
    ...
    },
    {
      ...
      generer() { const ang = pick([25, 35, 40, 55, 65]), hyp = randInt(7, 15); return { enonce: `Triangle rectangle. $\\widehat{B}=${ang}°$, hypoténuse $= ${hyp}$. Calcule le côté adjacent.`, reponse: round1(hyp * Math.cos(deg2rad(ang))), validation: 'nombre', tolerance: 0.05, _v: { ang, hyp } }; },
  ...
  quiz_bilan: [
    ...
    { type: 'saisie', question: 'Calcule un côté adjacent.', generer() { const ang = pick([20, 30, 40, 50, 60]), hyp = randInt(6, 14); return { question: `$\\widehat{B}=${ang}°$, hypoténuse $= ${hyp}$. Calcule l'adjacent (au dixième).`, reponse: round1(hyp * Math.cos(deg2rad(ang))), validation: 'nombre', tolerance: 0.05, explication: `adjacent $= ${hyp} \\times \\cos(${ang}°)$.` }; } },
