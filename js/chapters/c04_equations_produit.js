...
export default {
  ...
  exercices: [
    ...
    },
    {
      ...
      generer() {
        const a = randInt(2, 9);
        return { enonce: `$x^2 - ${a * a} = 0$`, reponse: [a, -a], validation: 'solutions', reponseTex: `x = ${a} \\text{ ou } x = ${-a}` };
      },
      indices: ['Reconnais une différence de carrés : $x^2 - b^2$.', '$x^2 - b^2 = (x-b)(x+b)$.', 'Puis applique le produit nul.'],
      correction_detaillee: (s) => `<p>$x^2 - b^2 = (x-b)(x+b) = 0$, d'où $x = b$ ou $x = -b$.</p>`,
    },
    {
  ...
  quiz_bilan: [
    ...
    { type: 'saisie', question: 'Résous $(x-3)(x-7) = 0$ (sépare par ;).', reponse: [3, 7], validation: 'solutions', explication: '$x=3$ ou $x=7$.' },
    ...
    { type: 'saisie', question: 'Résous $x^2 - 25 = 0$ (sépare par ;).', reponse: [5, -5], validation: 'solutions', explication: '$x^2-25=(x-5)(x+5)=0$, donc $x=5$ ou $x=-5$.' },
  ],
};
